...
describe("Patch", () => {
  ...
  describe("add", () => {
    ...
    it("should add a new object to a populated array", async () => {
      ...
      const book = await Book.findOne({name: "The Two Towers"});
  ...
  describe("replace", () => {
    ...
    it("should set a value on a populated path in a subdoc", async () => {
      ...
      const coauthor = await Author.findOne({_id: coauthor_id});
  ...
  describe("embedded arrays", () => {
    ...
    it("should patch array doc refs", async () => {
      ...
      const book1 = await Book.findOne({_id: book_id});
      const promise = book1.jsonPatch(patch);
      await promise;
      const changed_collaborator = await Author.findOne({ _id: collaborator_id});
