...
class JSONPatchMongoose {
  ...
  async add(item) {
    ...
    if (Array.isArray(parent)) {
      ...
      if (parent.isMongooseArray) {
        const array_schema = parent.$schema ? parent.$schema() : parent._schema;
        ...
        if (array_schema.options && //just
          ...
          array_schema.options.type[0].ref
        ) { //safe
          const model = mongoose.model(array_schema.options.type[0].ref);
  ...
  /**
     ...
     */
  setPath(path, value) {
    const path_info = this.path_info[path];
  ...
  async populatePath(path) {
    ...
    for (let i = 0; i <= parts.length; i++) {
      ...
      //If this is an ObjectId, it may or may not be a ref that needs to be populated
        ...
        const schema_type = relative_root.schema.path(relative_path);
      ...
      //if this is an array
        ...
        const array_schema = current_object.$schema ? current_object.$schema() : current_object._schema;
